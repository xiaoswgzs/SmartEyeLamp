...
//	printf("[d,3,1,小助手：]");
	while(1)
	{
        ...
        printf("[d,1,61,当前亮度为：%03d]",Get_Brightness_Set());
		printf("[d,1,81,护眼级调光频率：10KHz]");
		printf("[d,1,111,      CodeCat工作室,20]");
		dist = MyVL53L0X_GetDistance();
        if (abs(dist - Last_dist) > 5) 
		{
			OLED_ShowNum(3, 1, dist, 4);  
			printf("[d,1,1,距离：%03d cm]",dist/10);

			if (dist < 500 && Last_dist >= 500)
			{
				OLED_ShowString(4, 1, "Too close");
				printf("[d,1,41,小助手：                      ]");
				printf("[d,1,41,小助手：你靠太近啦，要离我50cm！]");
			}
			else if (dist >= 500 && Last_dist < 500)
			{
				OLED_ShowString(4, 1, "         ");
				printf("[d,1,41,小助手：                      ]");
				printf("[d,1,41,小助手：距离刚刚好，保持哦！]");
			}
